...
namespace lte {
	...
	std::vector<PrefLine> Bootstrapper::FileLines;
					...
					else if (choice == "exit")
					{
						if (category == "uuddlrlrbaStart")
						{
							...
							if (exitchoice == "save")
							{
								...
								data = newprefs;
								//flushes and prevents malformed preferences
	...
	void Bootstrapper::LoadPrefs(std::string filename)
	{
		...
		while (std::getline(file, line)) {
			...
			case ';':
				linestruct.key = line;
				linestruct.type = 2;
				FileLines.push_back(linestruct);
				linecount++;
				continue;
			case '[':
	...
	std::string Bootstrapper::SaveFile()
	{
		...
		//loads all into file lines
		for (const auto& cat_pair : data) {
			for (const auto& pref_pair : cat_pair.second) {
				FileLines[pref_pair.second.lineNum].value = pref_pair.second.value;
			}
		}
		...
		{
			switch (FileLines[i].type)
			{
			case 1:
				lines.emplace_back(FileLines[i].key + "=" + FileLines[i].value);
				break;
			default:
				lines.emplace_back(FileLines[i].key);
			}
		}
	...
	std::string Bootstrapper::GenerateFile()
	{
		...
		Insert(newFile, "write_frequency = 30");
